 ...
 */
#define QRS_QUALITY_MIN_MATERNAL 3.0f
#define QRS_QUALITY_MIN_FETAL    1.5f
...
/* Maximum coefficient of variation of the accepted RR series. Was an
 * inline literal at the consistency check. */
#define RR_CV_MAX              0.25f
 ...
 * values no longer add up to wall-clock time. These indices are handed to
 * the trend (with SAMPLE_PERIOD_MS) and become the beat timeline that
 * every DURATION is measured against -- the MLTV block boundaries and the
 * accel/decel episode lengths.
 *
...
#define ENVELOPE_TO_SIGNAL(n) \
    ...
    for (uint32_t i = 1U; i < (len - 1U); i++)
    {
         ...
         */
        if (have_last && ((i - decay_anchor) > stall_samples))
        {
                               ...
                               WombCareVitals_t   *output_vitals)
{
    ...
    if (maternal_ok)
    {
        ...
        if (n >= 3U)
        {
            ...
            maternal_bpm = median_of(fhr_bpm, n, flat_abdom_ecg);
     ...
     *
     * beat_idx is compacted alongside, so the beat timeline handed to
     * the trend stays correct after this loop. Durations must come from
     * it rather than from accumulating RR values: this loop drops beats,
     * and the surviving RR values no longer sum to wall-clock time.
     *-------------------------------------------------------------*/
