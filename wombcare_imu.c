...
#include <string.h>
#include "sl_imu.h"
#include "sl_sleeptimer.h" // Fixes the delay function error
#include "app_log.h"       // Fixes the app_log_info error
/*
...
void wombcare_imu_reset(void)
{
    ...
    memset(&imu_result, 0, sizeof(imu_result));
}
...
{
    /* 1. Turn on the power pin (Keep this, just in case!) */
    GPIO_PinOutSet(
        IMU_ENABLE_PORT,
        IMU_ENABLE_PIN);

    /* 2. Give the IMU chip 50ms to physically boot up */
    sl_sleeptimer_delay_millisecond(50);

    /* 3. Initialize the SPI/I2C communication */
    sl_imu_init();

    /* 4. THE MISSING LINK: Turn on the accelerometer engine at 26 Hz! */
    sl_imu_configure(26.0f); 
}
void wombcare_imu_power_off(void)
{
    /* Put the IMU hardware to sleep */
    sl_imu_deinit();
...
static bool imu_read_acceleration(
    ...
    float *accel_z)
{
    /* Check if the hardware actually has new data ready to be read */
    if (!sl_imu_is_data_ready()) {
        return false;
    }

    /* Tell the hardware to lock in the new reading */
    sl_imu_update();

    /* Pull the real X, Y, Z data from the chip */
    int16_t avec[3];
    sl_imu_get_acceleration(avec);

    /* --- DEBUG PRINT (Once every ~26 samples = 1 second) --- */
    static int counter = 0;
    if (counter++ % 26 == 0) {
        app_log_info("RAW IMU - X:%d  Y:%d  Z:%d\r\n", avec[0], avec[1], avec[2]);
    }

    /* Convert milli-G's to float G's */
    *accel_x = (float)avec[0] / 1000.0f;
    *accel_y = (float)avec[1] / 1000.0f;
    *accel_z = (float)avec[2] / 1000.0f;
