...
//
// WombCare: MUST be FSRCO, not the EM01GRPACLK default.
// EM01GRPACLK stops in EM2. This project runs the Power Manager with
// deep sleep, so the core drops to EM2 between BLE/sleeptimer events.
// On EM01GRPACLK the LETIMER keeps counting and PRS keeps pulsing, but
// the IADC has no clock and never converts: the scan FIFO stays empty
// and the LDMA is never requested. FSRCO keeps running in EM2.
#ifndef SL_CLOCK_MANAGER_IADCCLK_SOURCE
#define SL_CLOCK_MANAGER_IADCCLK_SOURCE    CMU_IADCCLKCTRL_CLKSEL_FSRCO
#endif
